...
export const AdminDashboard: React.FC = () => {
  ...
  const pendingVerification = teachers.filter(
    (t) =>
      t.verificationStatus.status === "pending" ||
        ...
        <div className="border-b border-white/10 pb-6 flex justify-between items-end">
          <div>
            ...
            <p className="text-white/60 text-sm">
              Monitor revenue, enforce AI moderation policies, and verify global
              teacher identities.
            </p>
          </div>
          <div className="flex gap-4">
            ...
            <button
              ...
              className={`px-6 py-2 rounded-xl text-sm font-bold transition-all flex items-center gap-2 ${activeAdminTab === "store" ? "bg-[white] text-[black] shadow-lg shadow-[white]/20" : "bg-white/5 hover:bg-white/10 text-white/70"}`}
            >
              Store Management
            </button>
            <button
        ...
        {activeAdminTab === "overview" ? (
          <>
            ...
            <div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                <StatCard
                  ...
                  value={`$${analytics.totalRevenueUSD.toLocaleString()}`}
                  icon={<DollarSign />}
                  highlight
                />
                <StatCard
                  title={`Company Commission (${adminSettings.commissionPercentage}%)`}
                  value={`$${analytics.platformEarningsUSD.toLocaleString()}`}
                  icon={<TrendingUp />}
                />
                <StatCard
                  title={`Teacher Payouts (${100 - adminSettings.commissionPercentage}%)`}
                  value={`$${analytics.teacherPayoutsUSD.toLocaleString()}`}
                  icon={<Users />}
                />
                <StatCard
              ...
              <div className="mt-8 bg-[#18181b] rounded-[32px] border border-white/10 p-6 md:p-8 shadow-xl">
                <h3 className="text-lg font-bold mb-6 flex items-center gap-2">
                  <TrendingUp className="w-5 h-5 text-[white]" /> Monthly
                  Revenue & Commission
                </h3>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              ...
              <div className="bg-[#18181b] rounded-[32px] border border-white/10 p-6 md:p-8 shadow-xl">
                <h3 className="text-lg font-bold mb-6 flex items-center gap-2">
                  <Shield className="w-5 h-5 text-blue-400" /> Identity
                  Verification Queue
                </h3>
                {pendingVerification.length === 0 ? (
                  <p className="text-xs text-white/50">
                    All teacher KYC applications have been processed.
                  </p>
                ) : (
                  <div className="space-y-4">
                    {pendingVerification.map((t) => (
                      <div
                        key={t.id}
                        className="bg-white/5 p-4 rounded-xl border border-white/5 flex flex-col sm:flex-row sm:items-center justify-between gap-4"
                      >
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-[10px] text-white/50">
                              {t.country} • ID Submitted & Selfie Check Pending
                            </p>
                          </div>
                        </div>
              ...
              <div className="bg-[#18181b] rounded-[32px] border border-white/10 p-6 md:p-8 shadow-xl">
                <h3 className="text-lg font-bold mb-6 flex items-center gap-2">
                  <AlertTriangle className="w-5 h-5 text-red-400" /> AI
                ...
                <div className="space-y-4">
                  {reports.map((r) => (
                    <div
                      key={r.id}
                      className="bg-white/5 p-4 rounded-xl border border-red-500/20 flex flex-col gap-3 relative overflow-hidden"
                    >
                      ...
                      <div className="flex items-center justify-between mt-2">
                        <span className="text-[10px] text-red-400 font-bold">
                          AI Fraud Confidence:{" "}
                          {(r.aiFraudConfidence! * 100).toFixed(0)}%
                        </span>
              ...
              <div className="bg-[#18181b] rounded-[32px] border border-white/10 p-6 md:p-8 shadow-xl lg:col-span-2">
                <h3 className="text-lg font-bold mb-6 flex items-center gap-2">
                  <DollarSign className="w-5 h-5 text-green-400" /> Pending
                  Withdrawal Requests
                </h3>
                    ...
                    <tbody className="divide-y divide-white/5">
                      {withdrawals.map((w) => (
                        ...
                        >
                          ...
                          <td className="py-4 px-2 font-mono text-white/60">
                            {w.accountDetails}
                          </td>
                          <td className="py-4 px-2">
                            <span
                              className={`px-2 py-1 rounded text-[9px] uppercase font-bold ${w.status === "pending" ? "bg-yellow-500/20 text-yellow-400" : w.status === "rejected" ? "bg-red-500/20 text-red-400" : "bg-green-500/20 text-green-400"}`}
                            >
                              {w.status}
                            </span>
                          </td>
          ...
          <div className="bg-[#18181b] rounded-[32px] border border-white/10 p-6 md:p-8 shadow-xl max-w-4xl">
            ...
            <form onSubmit={handleSaveSettings} className="space-y-8">
              <div className="space-y-4">
                <h4 className="text-sm font-bold text-white/70 uppercase tracking-widest border-b border-white/5 pb-2">
                  Platform Economics
                </h4>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    ...
                    <p className="text-[10px] text-white/40 mt-1">
                      Platform takes {formSettings.commissionPercentage}% and
                      teachers take {100 - formSettings.commissionPercentage}%.
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-4">
                <h4 className="text-sm font-bold text-white/70 uppercase tracking-widest border-b border-white/5 pb-2">
                  Admin Payment Accounts (Payout Source)
                </h4>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    <label className="block text-xs font-bold text-white/60 mb-2">
              ...
              <div className="pt-6 border-t border-white/5 flex justify-end">
                ...
                >
                  {isSaving ? (
                    ...
                    <>
                      <Save className="w-4 h-4" /> Save Settings Securely
                    </>
                  )}
                </button>
              </div>
...
const StatCard: React.FC<{
  ...
  <div
    className={`p-6 rounded-[24px] border ${highlight ? "bg-gradient-to-br from-[#112240] to-[#0A192F] border-[white]/50 shadow-[white]/20" : "bg-[#18181b] border-white/5"} shadow-xl relative overflow-hidden`}
  >
    <div className="flex items-center justify-between mb-4">
      ...
      <div
        className={`p-2 rounded-xl ${highlight ? "bg-[white]/20 text-[white]" : "bg-white/5 text-white/60"}`}
      >
        {icon}
      </div>
    </div>
    <h3
      className={`text-2xl lg:text-3xl font-bold tracking-tight ${highlight ? "text-[white]" : "text-white"}`}
    >
      {value}
    </h3>
  </div>
