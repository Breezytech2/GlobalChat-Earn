    ...
    <header className="h-20 border-b border-white/5 bg-black/80 backdrop-blur-md flex items-center justify-between px-8 flex-shrink-0 z-30 select-none">
      ...
      <nav className="hidden md:flex items-center gap-8 text-sm font-medium text-zinc-400">
        ...
        <button
          ...
          className={`transition-all ${activeTab === "store" ? "text-white border-b-2 border-white py-7 font-semibold" : "hover:text-white"}`}
        >
          Store
        </button>
        <button
      ...
      <div className="flex items-center gap-5">
        ...
        {currentUser.id !== "u_student_1" && (
          <div
            className={`relative cursor-pointer transition-all ${activeTab === "settings" ? "text-white" : "text-zinc-300 hover:opacity-80"}`}
